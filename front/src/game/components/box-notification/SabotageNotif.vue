  ...
  <div>
    <div class="box-notification-header">
      ...
      <div
        class="name"
        v-html="$tmd(
          `notification.box.sabotage.title`,
    ...
    <div class="box-notification-tabs">
      ...
      <div class="box-notification-tab-item">
        ...
        <div
          ...
          <character-card
            :character="data.target.current"
            :theme="theme(data.target.current.owner.faction)" />
        </div>
        <div
          v-if="tabs[activeTab].includes('target-army')"
          class="box-notification-bloc is-army"
          :class="`f-${theme(data.target.current.owner.faction)}`">
          <army
            :has-header="false"
            :context="'display'"
            :character="data.target.previous"
            :diff="data.target.current" />
        </div>
      </div>
    </div>
  </div>

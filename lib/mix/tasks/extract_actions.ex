...
defmodule Mix.Tasks.ExtractActions do
  @moduledoc """
  Generates "custom action nodes" for the [behavior tree
  editor](https://github.com/adobe/behavior_tree_editor).

  This will scan all actions defined in the supplied `actions-dir/actions.ex` file to build
  ...
  * `actions-dir` - [required] Path to the directory containing `actions.ex`.
  * `module-base` - [optional] If all of your actions start with a common prefix (Ex:
  ...
  `module_base` option in `SystemAI.Parser.parse!/2` to ensure the stripped base
  gets re-appended.
  * `bt-json-file` - [optional] Location of behavior tree editor project file.
  ...
  def run(args) do
    ...
    actions_dir = Keyword.get(flags, :actions_dir) || raise "You must specify the \"actions_dir\" parameter"
